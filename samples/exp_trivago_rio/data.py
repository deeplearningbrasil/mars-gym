...
class PrepareHistoryInteractionData(BasePySparkTask):
    ...
    def group_history_data(self, df):
        ...
        pad_fix_length = F.udf(
            lambda arr: arr[:self.window_hist] + [''] * (self.window_hist - len(arr[:self.window_hist])), 
    ...
    def main(self, sc: SparkContext, *args):
        ...
        df = spark.read.csv(self.input()[0][0].path, header=True, inferSchema=True).orderBy("timestamp")
...
class PrepareTrivagoDataFrame(BasePrepareDataFrames):
    ...
    def requires(self):
        return PrepareHistoryInteractionData(window_hist=self.window_hist), PrepareMetaData()
