...
class PrepareDataset(luigi.Task):
    ...
    def add_history_buys(self, df):
        ...
        for g, df_g in df.groupby('SessionID'):
            list_of_indexes=[]
            df_g['ItemID'].rolling(self.history_window, min_periods=1).apply((lambda x: list_of_indexes.append(x.astype(int).tolist()) or 0), raw=False)
            df_g['ItemID_history'] = list_of_indexes
            df_g['ItemID_history'] = df_g['ItemID_history'].shift(1)
            
            # add padding 
            Item_prev = df_g['ItemID_history'].apply(lambda x: [] if x is np.nan else x)\
                                            .apply(lambda x: ([''] * (self.history_window - len(x)) + x)[:self.history_window])
    ...
    def add_available_items(self, df):
        ...
        df['available_items'] = df['ItemID'].apply(lambda item_id: random.sample(all_items, self.size_available_list - 1) + [item_id])
        return df
    ...
    def run(self):
        ...
        df = pd.read_csv(self.input()[0].path, parse_dates=[1], header=None).dropna().sort_values([1])
...
class PrepareInteractionDataFrame(BasePrepareDataFrames):
    ...
    def requires(self):
        return PrepareDataset(history_window=self.history_window, size_available_list=self.size_available_list)
