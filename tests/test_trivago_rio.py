...
#@patch("exp_trivago_rio.data.BASE_DIR", 'tests/output/trivago_rio')
#@patch("exp_trivago_rio.data.DATASET_DIR", 'tests/output/trivago_rio/dataset')
#@patch("exp_trivago_rio.data.OUTPUT_PATH", 'tests/output')
...
class TestTrivagoRio(unittest.TestCase):
  ...
  def test_interactive_and_evaluation(self):
    ## PYTHONPATH="." luigi --module exp_trivago_rio.simulation TrivagoModelInteraction --project trivago_rio --n-factors 100 --metrics '["loss"]'  --obs-batch-size 1000 --batch-size 200 --num-episodes 1 --val-split-type random --full-refit --bandit-policy epsilon_greedy --bandit-policy-params '{"epsilon": 0.1}' --epochs 100 --seed 42
