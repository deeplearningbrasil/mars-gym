...
class FillTrivagoPropensityScoreMixin(object, metaclass=abc.ABCMeta):
    fill_ps_strategy: str = luigi.ChoiceParameter(
        choices=[
            "per_pos_item_idx",
            "per_item",
    ...
    def _create_dict_of_ps_per_pos_item_idx(
    ...
    def fill_ps_per_item_in_first_pos(self, df: pd.DataFrame, pool: Pool):
        ps_per_item_in_first_pos = {
            item: np.sum(
                (self.ps_base_df[self.item_column] == item)
                & (self.ps_base_df["pos_item_idx"] == 0)
            )
    ...
    def fill_ps_per_item_given_pos(self, df: pd.DataFrame, pool: Pool):
        ps_per_item_given_pos: Dict[Tuple[int, int], float] = {
            (pos_item_idx, item): np.sum(
                (self.ps_base_df[self.item_column] == item)
                & (self.ps_base_df["pos_item_idx"] == pos_item_idx)
            )
        ...
        df[self.propensity_score_column] = df.apply(
            lambda row: ps_per_item_given_pos.get(
                (row["pos_item_idx"], row[self.item_column]), 0.0
            ),
            axis=1,
        )

    def fill_ps_per_logistic_regression_of_pos_item_idx_and_item(
        self, df: pd.DataFrame, pool: Pool
    ):
        ...
        model.fit(
            encoder.transform(train_df.values),
            self.ps_base_df[self.output_column].values,
        )

        df[self.propensity_score_column] = model.predict_proba(
            encoder.transform(df[["pos_item_idx", self.item_column]].values)
        )[:, 1]

    def fill_ps_per_logistic_regression_of_pos_item_idx_and_item_ps(
        self, df: pd.DataFrame, pool: Pool
    ):
        ps_per_pos_item_idx = self._create_dict_of_ps_per_pos_item_idx(self.ps_base_df)
        ps_per_item = self._create_dict_of_ps_per_item(self.ps_base_df)
        ...
        train_df = pd.DataFrame(
            data={
                "ps_per_pos_item_idx": self.ps_base_df["pos_item_idx"].apply(
                    lambda pos_item_idx: ps_per_pos_item_idx.get(pos_item_idx, 0.0)
                ),
                "ps_per_item": self.ps_base_df[self.item_column].apply(
                    lambda item_idx: ps_per_item.get(item_idx, 0.0)
                ),
            }
        )
        ...
        test_df = pd.DataFrame(
            data={
                "ps_per_pos_item_idx": df["pos_item_idx"].apply(
                    lambda pos_item_idx: ps_per_pos_item_idx.get(pos_item_idx, 0.0)
                ),
                "ps_per_item": df[self.item_column].apply(
                    lambda item_idx: ps_per_item.get(item_idx, 0.0)
                ),
            }
        )
    ...
    def fill_ps(self, df: pd.DataFrame, pool: Pool):
        {
            ...
            "per_logistic_regression_of_pos_item_idx_and_item": self.fill_ps_per_logistic_regression_of_pos_item_idx_and_item,
            "per_logistic_regression_of_pos_item_idx_and_item_ps": self.fill_ps_per_logistic_regression_of_pos_item_idx_and_item_ps,
