...
import numpy as np
from mars_gym.model.transformer import *
import copy
...
class SimpleCNNModel(nn.Module):
    def __init__(
        ...
        self.convs1 = nn.ModuleList(
            [
                nn.Conv2d(1, num_filters, (K, n_factors * context_embs))
    ...
    def forward(
        ...
        interaction_item_image_emb = self.pe(
            self.item_embeddings(list_interaction_item_image_idx)
        )
        interaction_item_info_emb = self.pe(
            self.item_embeddings(list_interaction_item_info_idx)
        )
        interaction_item_rating_emb = self.pe(
            self.item_embeddings(list_interaction_item_rating_idx)
        )
        interaction_item_deals_emb = self.pe(
            self.item_embeddings(list_interaction_item_deals_idx)
        )
        ...
        change_of_sort_order_emb = self.pe(
            self.word_embeddings(list_change_of_sort_order)
        )
        search_for_destination_emb = self.pe(
            self.word_embeddings(list_search_for_destination)
        )
        filter_selection_emb = self.pe(self.word_embeddings(list_filter_selection))

        context_session_emb = torch.cat(
            (
                actions_type_emb,
                clickout_item_emb,
...
class SimpleRNNModel(nn.Module):
    def __init__(
        ...
        continuos_features = (
            ...
            + metadata_size
            + filter_size
            + (recurrence_hidden_size * window_hist_size * context_embs)
        )  # Price + Meta
    ...
    def forward(
        ...
        # Categorical embs
        actions_type_out, _ = self.rnn12(
            self.action_type_embeddings(list_action_type_idx)
        )

        # Item embs
        clickout_item_out, _ = self.rnn1(self.item_embeddings(list_clickout_item_idx))
        interaction_item_image_out, _ = self.rnn2(
            self.item_embeddings(list_interaction_item_image_idx)
        )
        interaction_item_info_out, _ = self.rnn3(
            self.item_embeddings(list_interaction_item_info_idx)
        )
        interaction_item_rating_out, _ = self.rnn4(
            self.item_embeddings(list_interaction_item_rating_idx)
        )
        interaction_item_deals_out, _ = self.rnn5(
            self.item_embeddings(list_interaction_item_deals_idx)
        )
        search_for_item_out, _ = self.rnn6(
            self.item_embeddings(list_search_for_item_idx)
        )

        # NLP embs
        search_for_poi_out, _ = self.rnn7(self.word_embeddings(list_search_for_poi))
        change_of_sort_order_out, _ = self.rnn8(
            self.word_embeddings(list_change_of_sort_order)
        )
        search_for_destination_out, _ = self.rnn9(
            self.word_embeddings(list_search_for_destination)
        )
        filter_selection_out, _ = self.rnn10(
            self.word_embeddings(list_filter_selection)
        )
        ...
        context_session_emb = torch.cat(
            (
                ...
                search_for_item_out,
                search_for_poi_out,
                change_of_sort_order_out,
                search_for_destination_out,
                filter_selection_out,
            ),
            dim=1,
        )
...
class SimpleCNNTransformerModel(nn.Module):
    def __init__(
        ...
        self.layers = self.get_clones(
            EncoderLayer(
                n_factors * context_embs, self.transform_heads, dropout=dropout_prob
            ),
            self.transform_n,
        )
