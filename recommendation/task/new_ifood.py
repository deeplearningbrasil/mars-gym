...
from recommendation.task.data_preparation.new_ifood import PrepareNewIfoodIndexedOrdersTestData, \
    ListAccountMerchantTuplesForNewIfoodIndexedOrdersTestData, CheckDataset, IndexTrainDataset
...
def _generate_relevance_list_from_merchant_scores(ordered_merchant_idx: int, merchant_idx_list: List[int],
    ...
    scores = list(map(lambda merchant_idx: scores_per_merchant[merchant_idx] if merchant_idx in scores_per_merchant  else 0, merchant_idx_list))
    sorted_merchant_idx_list = [merchant_idx for _, merchant_idx in
                                sorted(zip(scores, merchant_idx_list), reverse=True)]
    return [1 if merchant_idx == ordered_merchant_idx else 0 for merchant_idx in sorted_merchant_idx_list]


def _generate_relevance_list_from_merchant_and_account_scores(ordered_merchant_idx: int, ordered_account_idx: int, merchant_idx_list: List[int],
                                                  scores_per_merchant: Dict[int, float]) -> List[int]:
    scores = list(map(lambda merchant_idx: scores_per_merchant[(merchant_idx, ordered_account_idx)] if (merchant_idx, ordered_account_idx) in scores_per_merchant  else 0, merchant_idx_list))
    sorted_merchant_idx_list = [merchant_idx for _, merchant_idx in
                                sorted(zip(scores, merchant_idx_list), reverse=True)]
    return [1 if merchant_idx == ordered_merchant_idx else 0 for merchant_idx in sorted_merchant_idx_list]
...
class GenerateRelevanceListsForIfoodModel(BaseEvaluationTask):
    ...
    def requires(self):
        return PrepareNewIfoodIndexedOrdersTestData(), \
               ListAccountMerchantTuplesForNewIfoodIndexedOrdersTestData()
    ...
    def _evaluate_account_merchant_tuples(self) -> Dict[Tuple[int, int], float]:
        ...
        for indices in tqdm(chunks(range(len(df)), self.batch_size),
            ...
            batch_tensor = torch.sparse_coo_tensor(
                ...
                size=[len(rows), self.model_training.n_items],
                dtype=torch.float32).to(self.model_training.torch_device)
...
class EvaluateIfoodCVAEModel(EvaluateIfoodModel):
    def requires(self):
        return GenerateReconstructedInteractionMatrix(model_module=self.model_module, model_cls=self.model_cls,
                                                      model_task_id=self.model_task_id, variational=True)
...
class GenerateMostPopularRelevanceLists(luigi.Task):
    ...
    def requires(self):
        return CheckDataset(), \
               PrepareNewIfoodIndexedOrdersTestData()
    ...
    def get_scores(self):
        ...
        interactions_df: pd.DataFrame = pd.read_parquet(self.input()[0][0].path)
...
class GenerateMostPopularPerUserRelevanceLists(GenerateMostPopularRelevanceLists):

    def get_scores(self):
        print("Reading the interactions DataFrame...")
        interactions_df: pd.DataFrame = pd.read_parquet(self.input()[0][0].path)
        scores: pd.Series = interactions_df.groupby(["merchant_idx", "account_idx"])["buys"].sum()
        scores_dict: Dict[int, float] = {merchant_account_idx: score for merchant_account_idx, score
                                         in tqdm(zip(scores.index, scores),
                                                 total=len(scores))}
        return scores_dict
    ...
    def run(self):
        ...
        orders_df["relevance_list"] = list(tqdm(
            starmap(functools.partial(_generate_relevance_list_from_merchant_and_account_scores, scores_per_merchant=scores_dict),
                    zip(orders_df["merchant_idx"], orders_df["account_idx"], orders_df["merchant_idx_list"])),
            total=len(orders_df)))
...
class EvaluateMostPopularPerUserIfoodModel(EvaluateIfoodModel):
    ...
    def requires(self):
        return GenerateMostPopularPerUserRelevanceLists()