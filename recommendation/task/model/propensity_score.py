...
class FillPropensityScoreMixin(object, metaclass=abc.ABCMeta):
    ...
    def fill_ps(self, df: pd.DataFrame, pool: Pool):
        ...
        trial = Trial(self.policy_estimator.get_trained_module(),
                      criterion=lambda *args: torch.zeros(1, device=self.policy_estimator.torch_device,
                                                          requires_grad=True)) \
        ...
        params = zip(item_indices, probas, df[self.available_arms_column]) \
            if self.available_arms_column else zip(item_indices, probas)
