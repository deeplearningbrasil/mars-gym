...
from recommendation.model.trivago.trivago_models import TestModel, SimpleCNNModel, SimpleRNNModel, SimpleLinearModel, \
...
class TrivagoModelTraining(FillTrivagoPropensityScoreMixin, FillPropensityScoreMixin, TrivagoModelTrainingMixin,
                           BaseTorchModelWithAgentTraining):
    ...
    def evaluate(self):
        if self.project_config.dataset_class == InteractionsDataset:
            ...
            df_eval['item_idx'] = df_eval.apply(
                lambda row: int(np.max(np.array(row['clicked']) * np.array(row['list_item_idx']))), axis=1)

            def sort_and_bin(row):
                list_sorted, score = zip(
                    *sorted(zip(row['list_item_idx'], row['list_score']), key=lambda x: x[1], reverse=True))
