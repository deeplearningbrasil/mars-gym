...
from recommendation.task.data_preparation.base import (
    BasePySparkTask,
...
class BaseDir(luigi.Config):
    dataset_processed_path = luigi.Parameter(
        default=os.path.join(BASE_DIR, "dataset"), is_global=True
    )
    dataset_raw_path = luigi.Parameter(
        default=os.path.join(BASE_DIR, "ufg_dataset_all"), is_global=True
    )
    embedding = luigi.IntParameter(
        default=os.path.join("output", "embeddings"), is_global=True
    )
...
class CheckDataset(luigi.Task):
    def output(self):
        return (
            luigi.LocalTarget(
                os.path.join(BaseDir().dataset_raw_path, "info_availability")
            ),
            luigi.LocalTarget(
                os.path.join(BaseDir().dataset_raw_path, "info_delivery_time")
            ),
            ...
            luigi.LocalTarget(
                os.path.join(BaseDir().dataset_raw_path, "info_restaurant")
            ),
...
class CleanSessionDataset(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "info_session_cleaned_s=%d" % (self.sample_size),
            )
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        df_only_visit = (
            ...
            .sum()
            .withColumnRenamed("sum(buy)", "buy")
            .filter(col("buy") == 0)
        )

        # union buys with visits without visitsBuy
        df = df_buy.select(df.columns).union(df_only_visit.select(df.columns))
...
class AddAdditionallInformationDataset(BasePySparkTask):
    ...
    def add_history_buy_visits(self, df):
        ...
        win_total = (
            Window.partitionBy("merchant_id", "account_id")
            .orderBy("click_timestamp")
            .rangeBetween(Window.unboundedPreceding, 0)
        )
        ...
        df = (
            df.withColumn(
                "user_total_buys", F.sum(col("buy")).over(win_user_total) - col("buy")
            )
            .withColumn(
                "user_total_visits",
                F.sum(col("visit")).over(win_user_total) - col("visit"),
            )
            .withColumn(
                "item_total_buys", F.sum(col("buy")).over(win_item_total) - col("buy")
            )
            .withColumn(
                "item_total_visits",
                F.sum(col("visit")).over(win_item_total) - col("visit"),
            )
            .withColumn("hist_buys", F.sum(col("buy")).over(win_total) - col("buy"))
            .withColumn(
                "hist_visits", F.sum(col("visit")).over(win_total) - col("visit")
            )
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        availability_df = availability_df.withColumn(
            "day_of_week", availability_df["day_of_week"] - 1
        )
        ...
        session_df = session_df.withColumn(
            "click_timestamp_brt",
            from_utc_timestamp("click_timestamp", "America/Sao_Paulo"),
        )
        session_df = session_df.withColumn(
            "click_time", date_format(session_df.click_timestamp_brt, "HH:mm:ss")
        )
        session_df = session_df.withColumn(
            "day_of_week", date_to_day_of_week_udf(session_df.dt_partition)
        )
        session_df = session_df.withColumn(
            "day_of_month", date_to_day_of_month_udf(session_df.dt_partition)
        )
        session_df = session_df.withColumn(
            "shift", datetime_to_shift_udf(session_df.click_timestamp_brt)
        )
...
class PrepareRestaurantContentDataset(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(BaseDir().dataset_processed, "restaurants_with_contents.csv")
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        availability_df = availability_df.groupBy("merchant_id").agg(
            collect_set("day_of_week").alias("days_of_week"),
            collect_set("shift").alias("shifts"),
            collect_set("shift_idx").alias("shift_idx_list"),
        )

        menu_df = menu_df.groupBy("merchant_id").agg(
            collect_set("category_name").alias("category_names")
        )
        ...
        item_df = item_df.groupBy("merchant_id").agg(
            collect_list("item_text").alias("item_texts")
        )
        ...
        restaurant_df = restaurant_df.join(
            availability_df, on="merchant_id", how="left"
        )
        restaurant_df = restaurant_df.join(
            menu_df.select("merchant_id", "category_names", "menu_full_text"),
            on="merchant_id",
            how="left",
        )
...
class ProcessRestaurantContentDataset(luigi.Task):
    ...
    def output(self):
        return (
            luigi.LocalTarget(
                os.path.join(
                    BaseDir().dataset_processed,
                    "restaurants_with_processed_contents.csv",
                )
            ),
            luigi.LocalTarget(
                os.path.join(
                    BaseDir().dataset_processed, "restaurant_text_vocabulary.csv"
                )
            ),
            luigi.LocalTarget(
                os.path.join(BaseDir().dataset_processed, "dish_description.csv")
            ),
        )
    ...
    def load_stopwords(self):
        if not hasattr(self, "_stopwords"):
            with open(os.path.join(FILES_DIR, "stopwords-pt.txt"), "r") as file_handler:
                self._stopwords = [
                    unidecode(line.strip()) for line in file_handler.readlines()
                ]
    ...
    def run(self):
        ...
        context["menu_full_text"] = (
            context["menu_full_text"].str[: self.menu_text_length].replace(",", " ")
        )
        context["description"] = (
            context["description"].str[: self.description_text_length].replace(",", " ")
        )
        context["category_names"] = (
            context["category_names"].str[: self.category_text_length].replace(",", " ")
        )
        ...
        context["category_names"] = (
            context["category_names"].fillna("NAN").str.replace("|", " ")
        )
        ...
        vocab = (
            context["description"].values.tolist()
            + context["trading_name"].values.tolist()
            + context["category_names"].values.tolist()
            + context["menu_full_text"].values.tolist()
        )

        context = context.fillna("NAN").replace(r"^\s*$", "NAN", regex=True)
        tokenizer = StaticTokenizerEncoder(
            vocab, tokenize=self.tokenizer, min_occurrences=2
        )  # int(len(vocab)*0.01)
        ...
        for text_column in [
            ...
            context[text_column] = (
                tokenizer.batch_encode(context[text_column])[0]
                .cpu()
                .detach()
                .numpy()
                .tolist()
        ...
        context["restaurant_complete_info"] = context[
            restaurant_features
        ].values.tolist()
...
class BuildEmbeddingVocabulary(luigi.Task):
    ...
    description_text_length: int = luigi.IntParameter(default=200)
    category_text_length: int = luigi.IntParameter(default=250)
    filename: str = luigi.ChoiceParameter(
        choices=["cc.pt.300.vec"], default="cc.pt.300.vec"
    )

    def requires(self):
        return ProcessRestaurantContentDataset(
            menu_text_length=self.menu_text_length,
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed, "restaurant_text_vocabulary_vec.npy"
            )
        )
...
class SplitSessionDataset(BasePySparkTask):
    ...
    def output(self):
        return (
            ...
            luigi.LocalTarget(
                os.path.join(
                    BaseDir().dataset_processed,
                    "session_test_%.2f_s=%d" % (self.test_size, self.sample_size),
                )
            ),
        )

    def filter_train_session(
        self, df: pd.DataFrame, minimum_interactions: int
    ) -> pd.DataFrame:
        df_account_buy = (
            df.groupBy("account_id").agg(sum(df.buy).alias("count_buy")).cache()
        )
        df_account_buy = df_account_buy.filter(col("count_buy") >= minimum_interactions)

        df_merchant_visit = df.groupBy("merchant_id").count()
        df_merchant_visit = df_merchant_visit.filter(
            col("count") >= minimum_interactions
        )

        df = df.join(df_account_buy, "account_id", how="inner").join(
            df_merchant_visit, "merchant_id", how="inner"
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        n_test = (
            int(self.test_size)
            if self.test_size > 1
            else math.ceil(self.test_size * count)
        )
...
class GenerateIndicesForAccountsAndMerchantsDataset(BasePySparkTask):
    ...
    def output(self):
        return (
            luigi.LocalTarget(
                os.path.join(
                    BaseDir().dataset_processed,
                    "accounts_for_session_train_%d_s=%d.csv"
            ...
            luigi.LocalTarget(
                os.path.join(
                    BaseDir().dataset_processed,
                    "merchants_for_session_train_%d_s=%d.csv"
                    % (self.minimum_interactions, self.sample_size),
                )
            ),
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        df_all = df_train.select("account_id", "merchant_id").union(
            df_test.select("account_id", "merchant_id")
        )
...
class IndexAccountsAndMerchantsOfSessionTrainDataset(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "indexed_session_train_%.2f_k=%d_s=%d"
                % (self.test_size, self.minimum_interactions, self.sample_size),
            )
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        merchant_df = spark.read.csv(
            self.input()[1][1].path, header=True, inferSchema=True
...
class CreateGroundTruthForInterativeEvaluation(BasePySparkTask):
    ...
    def requires(self):
        return (
            IndexAccountsAndMerchantsOfSessionTrainDataset(
                test_size=0.0, minimum_interactions=self.minimum_interactions
            ),
            ProcessRestaurantContentDataset(),
        )
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed, "ground_truth_%s" % self.filter_dish
            )
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        if self.filter_dish != "all":
            df_dish = spark.read.csv(
                self.input()[1][2].path, header=True, inferSchema=True
            )
            df_dish = df_dish.filter(
                df_dish.dish_description.contains(self.filter_dish)
            )
...
class LoggingPolicyPsDataset(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "logging_policy_ps_session_train_%.2f_k=%d_s=%d.csv"
                % (self.test_size, self.minimum_interactions, self.sample_size),
            )
        )
    ...
    def _evaluate_logging_policy(self, orders_df):
        ...
        acc_mer_count = orders_df.groupBy("account_idx", "merchant_idx").agg(
            sum("visit").alias("account_merchant_visits")
        )

        # Group per Account
        acc_count = orders_df.groupBy("account_idx").agg(
            sum("visit").alias("account_visits")
        )
...
class CreateIntraSessionInteractionDataset(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "indexed_intra_session_train_%.2f_k=%d_s=%d"
                % (self.test_size, self.minimum_interactions, self.sample_size),
            )
        )
    ...
    def get_df_tuple_probs(self, df):
        ...
        df_join = df_tuple_count.join(
            df_count, df_tuple_count.merchant_idx_A == df_count._merchant_idx_A
        ).cache()
        df_join = df_join.withColumn("prob", col("count") / col("total"))

        df_join = (
            df_join.select("merchant_idx_A", "merchant_idx_B", "total", "prob")
            .withColumnRenamed("merchant_idx_A", "_merchant_idx_A")
    ...
    def main(self, sc: SparkContext, *args):
        ...
        df_pos = df.select(
            col("session_id").alias("_session_id"), posexplode(df.merchant_idxs)
        )
        ...
        df = (
            df.join(
                df_pos,
                (df.session_id == df_pos._session_id)
                & (df.merchant_idx_A == df_pos.col),
            )
        ...
        # Filter duplicates
        udf_join = F.udf(
            lambda s, x, y: "_".join(sorted([str(s), str(x), str(y)])), StringType()
        )
        df = df.withColumn(
            "key", udf_join("session_id", "merchant_idx_A", "merchant_idx_B")
        )
        ...
        df = df.join(
            df_probs,
            (df.merchant_idx_A == df_probs._merchant_idx_A)
            & (df.merchant_idx_B == df_probs._merchant_idx_B),
        )
...
class CreateInteractionDataset(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "interactions_train_%.2f_k=%d_s=%d"
                % (self.test_size, self.minimum_interactions, self.sample_size),
            )
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        # Group per Account
        train_df_grouped_users = (
            train_df.withColumn("user_total_buys", col("buy"))
            .withColumn("user_total_visits", lit(1))
            .groupBy("account_id")
            .agg(
                sum("user_total_buys").alias("user_total_buys"),
        ...
        # Group per Account and Merchant
        train_df_grouped = (
            train_df.withColumn("visit", lit(1))
            .groupBy("account_id", "account_idx", "merchant_id", "merchant_idx")
            .agg(
        ...
        train_df = train_df_grouped_users.join(
            train_df_grouped, "account_id", how="inner"
        )

        train_df = train_df.withColumn(
            "buy_prob", col("buys") / col("user_total_buys")
        ).na.fill(0.0)
        train_df = train_df.withColumn(
            "visit_prob", col("visits") / col("user_total_visits")
        ).na.fill(0.0)
...
class PrepareIfoodAccountMatrixWithBinaryBuysDataFrames(BasePrepareDataFrames):
    ...
    def _transform_data_frame(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = (
            df.groupby("account_idx")[["merchant_idx", "buys"]]
...
class PrepareIfoodMerchantMatrixWithBinaryBuysAndContentDataFrames(
    PrepareIfoodAccountMatrixWithBinaryBuysDataFrames
):
    def _transform_data_frame(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df[df["buys"] > 0]
        df = df[["account_idx", "merchant_idx", "buys"]]
...
class PrepareIfoodIndexedSessionTestData(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "indexed_session_test_data_%.2f_k=%d_s=%d.parquet"
                % (self.test_size, self.minimum_interactions, self.sample_size),
            )
        )
    ...
    def main(self, sc: SparkContext, *args):
        ...
        session_df = (
            session_df.join(account_df, "account_id", how="inner")
...
class PrepareIfoodIndexedOrdersTestData(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "indexed_orders_test_data_%.2f_k=%d_s=%d_f=%d.parquet"
                % (
                    self.test_size,
    ...
    def main(self, sc: SparkContext, *args):
        ...
        test_df = test_df.withColumn("mode_shift_idx", test_df.shift_idx).withColumn(
            "mode_day_of_week", test_df.day_of_week
        )

        # Filter idx only train set
        if not self.nofilter_iteractions_test:
            account_df = account_df.join(
                train_df.select("account_id").distinct(), "account_id", how="inner"
            )
            merchant_df = merchant_df.join(
                train_df.select("merchant_id").distinct(), "merchant_id", how="inner"
            )
        ...
        orders_df = (
            orders_df.join(account_df, "account_id", how="inner")
            .join(merchant_df, "merchant_id", how="inner")
            .dropDuplicates(
                [
                    "session_id",
            ...
            .select(test_df.columns)
            .cache()
        )

        # Join with merchant the same caracteristics to simulate merchants open
        availability_df = availability_df.join(
            merchant_df.select(["merchant_id", "merchant_idx"]),
            "merchant_id",
            how="inner",
        )
        availability_df = availability_df.withColumn(
            "open_time", date_format(availability_df.open, "HH:mm:ss")
        )
        availability_df = availability_df.withColumn(
            "close_time", date_format(availability_df.close, "HH:mm:ss")
        )
        availability_df = availability_df.withColumn(
            "open_time_minus_30_min",
            date_format(availability_df.open - expr("INTERVAL 30 MINUTES"), "HH:mm:ss"),
        )
        availability_df = availability_df.withColumn(
            "close_time_plus_30_min",
            date_format(
                availability_df.close + expr("INTERVAL 30 MINUTES"), "HH:mm:ss"
            ),
        )

        orders_df = (
            orders_df.join(
                availability_df.select(
                    [
                        "merchant_idx",
                        "day_of_week",
                        "open_time",
        ...
        orders_df = (
            orders_df.groupBy(test_df.columns)
            .agg(collect_set("merchant_idx").alias("merchant_idx_list"))
            .drop("merchant_idx")
        )

        orders_df = (
            orders_df.withColumn("count_visits", lit(count_visits))
...
class ListAccountMerchantTuplesForIfoodIndexedOrdersTestData(BasePySparkTask):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(
                BaseDir().dataset_processed,
                "account_merchant_tuples_from_test_data_%.2f_k=%d_s=%d_f=%d.parquet"
                % (
                    self.test_size,
                    self.minimum_interactions,
                    self.sample_size,
    ...
    def main(self, sc: SparkContext, *args):
        ...
        tuples_df = (
            df.union(df.withColumn("merchant_idx", explode(df.merchant_idx_list)))
