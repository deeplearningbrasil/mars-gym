...
class RecSysEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(self, dataset: pd.DataFrame, item_column: str, number_of_items: int, 
        ...
        if available_items_column:
            ...
            available_items = np.zeros((len(self._dataset), self._number_of_items), dtype=np.int8)
            non_zero_indices = [(i, available_item)
        ...
        if item_metadata is not None:
            observation_space[ITEM_METADATA_KEY] = spaces.Dict({key: spaces.Box(value.min(), value.max(),
