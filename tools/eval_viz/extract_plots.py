...
def export(args):
  input_legend      = ["bandit_policy", "bandit_policy_params.alpha"]
  ...
  for input_metrics in ['Cumulative Reward', 'Cumulative Mean Reward', 'Cumulative Window Mean Reward']:
    ...
    fig.write_image("tools/eval_viz/export/all_{}.png".format(input_metrics.replace(" ", "")))
  ...
  for group, rows in df.groupby("iteraction", sort=False):
    ...
    for input_metrics in ['Cumulative Reward', 'Cumulative Mean Reward', 'Cumulative Window Mean Reward']:
      ...
      fig.write_image("tools/eval_viz/export/{}_{}.png".format(group, input_metrics.replace(" ", "")))


    fig = plot_exploration_arm(df_group, title=group)
    fig.write_image("tools/eval_viz/export/{}_explorer.png".format(group))
    
    metrics.to_csv("tools/eval_viz/export/metrics.csv", index=False)
    params.to_csv("tools/eval_viz/export/params.csv", index=False)
    df_metrics_reward.to_csv("tools/eval_viz/export/metrics_reward.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--legend', default=500,  type=int)
